...
public class Kmeans {
	...
	public static final String CENT_PATH = "hdfs://192.168.192.100:9000/user/root/input_cents/cents.seq";
		...
		protected void reduce(Text key, Iterable<Text> values, Context context)
			...
			for (Text var : values) {
				...
				while (m.find()) {
					strs = m.group().split(":");
					if (strs.length == 2) {
						if(wordAndtfidf.containsKey("\"" + strs[0].replaceAll("\"", "").trim() + "\"")){//判断该单词是否曾经出现过
							Double sum = wordAndtfidf.get("\"" + strs[0].replaceAll("\"", "").trim() + "\"")+Double.parseDouble(strs[1].trim().replaceAll("\"",
									""));
							wordAndtfidf.put("\"" + strs[0].replaceAll("\"", "").trim() + "\"", sum);
						}else{
							wordAndtfidf.put("\"" + strs[0].replaceAll("\"", "").trim() + "\"",
									Double.parseDouble(strs[1].trim().replaceAll("\"",
											"")));
						}
					}
			...
			while(itr.hasNext()){
				Map.Entry<String, Double> entry1 = (Map.Entry<String, Double>) itr.next();
				if(entry1.getValue()/belongNum < 10E-4){
					itr.remove();
					wordAndtfidf.remove(entry1.getKey());
					//rm.add(entry1.getKey());
				}else{
					wordAndtfidf.put(entry1.getKey(), entry1.getValue()/belongNum);
				}
			}
			//wordAndtfidf.remove(rm);
...
//			context.write(key, this.cendroidTfidf);
	...
	public static void main(String[] args) throws Exception {
		...
		String[] ars = new String[] {
				"hdfs://192.168.192.100:9000/user/root/input_docs",
				"hdfs://192.168.192.100:9000/user/root/output_docs" };
